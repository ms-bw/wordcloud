...
// Extracting the label name and number of bursts from the topics array.
var dictionary = {}
topics.topics.forEach (
  function (topic) {
    dictionary[topic.label] = topic.burst
  }
)
...
// Figuring out largest burst and sorting condensedDictionary in numerical order, smallest - largest, in relation to number of bursts.
var sortedDictionary = condensedDictionary.sort (
...
var largestBurst = sortedDictionary[sortedDictionary.length -1].burst

// Creating buckets to place labels into according to number of bursts.
// Creating colours for labels according to sentimentScore.
var bucketDictionary = sortedDictionary.map (
  function (topic) {
    var bucket = "bucketOne"
    if (topic.burst >= 10) {
      bucket = "bucketTwo"
    }
    if (topic.burst >= 20) {
